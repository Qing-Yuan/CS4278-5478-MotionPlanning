...
class DSDA(Planner):
    ...
    def map_callback(self):
        ...
        for i in range(self.world_height):
            ...
            for j in range(self.world_width):
                self.aug_map[i].append(self.map[count])
                if self.map[count] == 100:
        ...
        for obstacle in obstacles:
            for k in range(1, self.inflation_ratio + 1):
                        if i - k >= 0:
                            self.aug_map[obstacle[0]-k][obstacle[1]] = 100
                        if i + k < self.world_height:
                            self.aug_map[obstacle[0]+k][obstacle[1]] = 100
                        if j - k >= 0:
                            self.aug_map[obstacle[0]][obstacle[1]-k] = 100
                        if j + k < self.world_width:
                            self.aug_map[obstacle[0]][obstacle[1]+k] = 100

        # print aug_map
        # s = [[str(e) for e in row] for row in self.aug_map]
    ...
    def generate_plan(self):
        ...
        source = self.get_current_discrete_state()
        g = 0
        ...
        while len(nodesToTry) > 0:
            ...
            for action in actions:
                current_v = action[0]
                current_w = action[1]
                next_pos = self.discrete_motion_predict(current_x, current_y, current_theta, current_v, current_w)
                if next_pos is None:
                    # collision
                    continue
                ...
                if next_pos in heap_dict:
                    if next_pos_g > heap_dict[next_pos]:
                        # added to heap before and g value was better
                        continue
                heapq.heappush(nodesToTry, (next_pos_f, (next_pos_g, next_pos, action)))
                heap_dict[next_pos] = next_pos_g
                parent[next_pos] = (current[1][1], action)
...
if __name__ == "__main__":
    ...
    np.savetxt("1_maze2_9_9.txt", result, fmt="%.2e")
